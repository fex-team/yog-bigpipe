...
BigPipe.prototype._renderPagelet = function(pagelet) {
    var fn = this.sources[pagelet.id];
    var eventname;

    if (!fn && this.sources.all) {
        fn = this.sources.all.bind(null, pagelet.id);
    }

    if (!fn && (eventname = 'onPagelet' + ucfirst(pagelet.id)) &&
            typeof pagelet.locals[eventname] === 'function' ) {

        fn = pagelet.locals[eventname];
    }

    if (!fn && typeof pagelet.locals['opPagelet'] === 'function') {
        fn = pagelet.locals['opPagelet'].bind(null, pagelet.id);
    }
    ...
    if (fn) {
        fn(function(err, val) {
            if (err) {
                // todo
                throw new Error('Prepare pagelet data error!');
                return;
            }
