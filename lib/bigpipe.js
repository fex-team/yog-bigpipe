...
BigPipe.prototype.bind = function (id, fn) {
    if (fn.length === 0) {
        // wrap promise style
        fn = (function (originFn) {
            return function (cb) {
                originFn().then(function (data) {
                    cb(null, data);
                }).catch(cb);
            }
        })(fn);
...
BigPipe.prototype.prepareAllSources = function (cb) {
    ...
    for (var id in sources) {
        if (sources.hasOwnProperty(id)) {
            promiseSources[id] = Promise.promisify(sources[id])().catch(function (err) {
                return {};
            });
        }
    };
    Promise.props(promiseSources).then(function (data) {
        me.sources = data;
        cb(null, data);
    });
}
